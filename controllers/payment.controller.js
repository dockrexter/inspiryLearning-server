    ...
    const create_payment_json = {
      ...
      redirect_urls: {
        return_url:
          ...
          "&messageId=" +
          req.body.messageId  + "&userId=" + req.user.id + `${req.body.mobile? "&mobile=1" : ""}`,
        cancel_url:
          paymentCancleUrlpro,
      },
      transactions: [
  ...
  paypal.payment.execute(
    ...
    async (error, payment) => {
      if (error) {
        ...
        const fbtokenClient = await getTokensByUserId(req.query.assignmentId);
        ...
        return req.query.mobile? res.end(`<html><head><link href="https://fonts.googleapis.com/css?family=Nunito+Sans:400,400i,700,900&display=swap" rel="stylesheet"></head><style> body {
          text-align: center;
          padding: 40px 0;
          background: #EBF0F5;
        } h1 {
        ...
        </div><h1>Success</h1><p>Your payment received successfully</p></div></body></html>`) : res.redirect(`${redirectClintUrl}/?id=${req.query.assignmentId }`);
      }
    }
  );
};
